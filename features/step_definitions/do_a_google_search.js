...
module.exports = function () {
  ...
  this.Then(/^I should see search results$/, function (callback) {
    ...
    this.waitFor(inputId, 5000, function (err) {

      if (err) {
        return callback.fail(new Error('Element ' + inputId + ' was not found after 5s'));
      }
  ...
  this.Then(/^I should see search results \(chainable version\)$/, function (callback) {
    var inputId = '#resultStats';

    this.waitFor(inputId, 5000).getText(inputId, function (err, text) {
